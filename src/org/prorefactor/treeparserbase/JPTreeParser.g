...
currentvaluefunc
  : #(CURRENTVALUE LEFTPAREN ID (COMMA ID)? (COMMA expression)? RIGHTPAREN )
  ;
...
def_modifiers
  : ( PRIVATE | PROTECTED | PUBLIC | STATIC | ABSTRACT | OVERRIDE | FINAL | SERIALIZABLE | NON_SERIALIZABLE )*
  ;
...
definepropertystate
  : #(  DEFINE def_modifiers PROPERTY ID AS datatype
      (extentphrase|initial_constant|NOUNDO|#(SERIALIZENAME QSTRING))*
...
nextvaluefunc
  : #(NEXTVALUE LEFTPAREN ID (COMMA ID)? (COMMA expression)? RIGHTPAREN )
  ;
